...
func TestRunRejectsProviderRootInsideCaptureState(t *testing.T) {
	...
	_, err := Run(context.Background(), RunOptions{
		Provider: ProviderClaude, OccurrenceID: "nested-provider-root",
		CaptureDir:   captureDir,
		ResultPath:   filepath.Join(t.TempDir(), "result.json"),
		ProviderRoot: filepath.Join(captureDir, "provider"),
		WorkDir:      t.TempDir(),
		Command:      []string{producer, "-p", "prompt"},
		Environment:  helperEnvironment(t.TempDir(), "claude-final", 0),
		Streams:      Streams{Stdout: &stdout, Stderr: io.Discard},
	...
	require.ErrorContains(t, err, "provider root must be outside")
	assert.Empty(t, stdout.String(), "the producer must not start")
	assert.NoDirExists(t, captureDir)
}
...
func TestRunRejectsContinuationInvocationsBeforeStarting(t *testing.T) {
	tests := []struct {
		name       string
		provider   Provider
		executable string
	...
	}{
		{
			...
			arguments: []string{"-p", "--continue", "prompt"},
		},
		{
		...
		},
		{
			...
			arguments: []string{"-p", "--fork-session", "prompt"},
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			captureDir := filepath.Join(t.TempDir(), "capture")
			_, err := Run(context.Background(), RunOptions{
				Provider: test.provider, OccurrenceID: "continuation",
				CaptureDir: captureDir, ResultPath: filepath.Join(t.TempDir(), "result.json"),
				ProviderRoot: t.TempDir(), WorkDir: t.TempDir(),
...
func TestRunCodexRetriesWhenChildChangesDuringCopy(t *testing.T) {
	...
	require.NoError(t, err)
	require.Eventually(t, func() bool {
		entries, err := os.ReadDir(filepath.Join(captureDir, stagingDirName))
		if err != nil {
			return false
		}
		for _, entry := range entries {
			...
			if statErr == nil && info.Size() > 1<<20 &&
				...
				return os.Chtimes(childPath, changed, changed) == nil
			}
		}
		return false
	}, 20*time.Second, time.Millisecond)
...
func writeCodexChildHelperOnDay(root string, day time.Time, large bool) {
